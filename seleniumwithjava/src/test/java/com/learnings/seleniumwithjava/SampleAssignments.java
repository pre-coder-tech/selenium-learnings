...
public class SampleAssignments {
    ...
    @Test(description = "Multi-Select Dropdown Handling")
    public void testMultiSelectDropdown() throws InterruptedException, IOException {
        ChromeOptions options = new ChromeOptions();
        options.addArguments("--incognito");
        options.addArguments("--start-maximized");
        WebDriver driver = new ChromeDriver(options);
        try {
            ...
            driver.findElements(By.cssSelector(".css-1wy0on6"))
                    .get(2).click();

            // The below peice of line is to get the outerHTML of the dropdown
            // as the dropdown HTML is not accesible in Chrome Dev Tools.
            System.out.println(
                    driver.findElement(
                            By.cssSelector(".css-26l3qy-menu"))
                            .getDomProperty("outerHTML"));

            // Selecting the options Green and Blue
            driver.findElements(By.cssSelector("div[class*='option']"))
                    .stream()
                    .filter(option
                            -> Stream.of("Green", "Blue")
                            .anyMatch(option.getText()::equals))
                    .forEach(option -> option.click());

            // Verifying if the options Green and Blue are selected
